...
class WumpusEnv(object):
    def __init__(self,observationAccu):
        ...
        self.stateSpace=[(agentPlace, wumpusPlace) for agentPlace in self.agentPlaceSpace for wumpusPlace in self.wumpusPlaceSpace if agentPlace!=wumpusPlace]+[((0,0), (0,-1))]
        self.actionSpace=['MoveUp', 'MoveDown', 'MoveLeft', 'MoveRight', 'ShootUp', 'ShootRight', 'ShootDown', 'ShootLeft']
        ...
        actionTransition={'MoveUp':moveFunction, 'MoveDown':moveFunction, 'MoveLeft':moveFunction, 'MoveRight':moveFunction,
                          'ShootUp':shootFunction, 'ShootRight':shootFunction, 'ShootDown':shootFunction, 'ShootLeft':shootFunction}
        ...
        movingReward=lambda s, a, sPrime: movingCost*(sPrime!=((0, 0), (0, -1)))+losingPenalty*(sPrime==((0, 0), (0, -1)))*(s!=((0, 0), (0, -1)))
        rewardTable={'MoveUp':movingReward, 'MoveDown':movingReward, 'MoveLeft':movingReward, 'MoveRight':movingReward,
                     'ShootUp':shootingReward, 'ShootRight':shootingReward, 'ShootDown':shootingReward, 'ShootLeft':shootingReward}
        rewardFunc=lambda s, a, sPrime: rewardTable[a](s, a, sPrime)

        rewardMatrix=np.array([[[rewardFunc(s, a, sPrime) for sPrime in self.stateSpace] for a in self.actionSpace] for s in self.stateSpace])
        ...
        observationMatrix=np.array([[[observationFunc(sPrime, a, o) for o in self.observationSpace] for a in self.actionSpace] for sPrime in self.stateSpace])
