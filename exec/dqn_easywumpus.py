...
class Train(object):
    ...
    def __call__(self, model, target_model, memory, simulator):
        ...
        for episode in range(self.maxEpisodes):
            ...
            observation = simulator.observation(state, 0)
            e = self.getEpsilon()
            total_reward = 0
            for step in range(self.maxSteps):
                model, target_model, memory, state, observation, terminal, reward = self.trainOneStep(model,
                                                                                                      target_model,
                                                                                                      memory, state,
                                                                                                      observation, e)
                total_reward += reward
...
def main():
    ...
    simulateOneStep = SimulateOneStep(simulator.transition, simulator.reward, simulator.observation,
