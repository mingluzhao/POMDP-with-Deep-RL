...
def runOneTimeStep(actorCritic,currentMemory,env,cumulativeReward, episode_step):
...
def main():
    ...
    for j in range(config["totalTrainStep"]):
        ...
        for step in range(config["numStepBeforeTrain"]):
            ...
            policyReturn,currentMemory,masks,reward,cumulativeReward,episode_step,done= runOneTimeStep(actorCritic,\
