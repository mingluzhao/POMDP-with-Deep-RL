...
import cartpole_dqn_torch as target


# The code below test learnbackprop (a little bit complicated)
...
class Build_Test_Model(target.nn.Module):
    ...
    def __init__(self):
        ...
        target.T.nn.init.constant_(self.nn_layers[0].weight, 2)
        self.device = target.T.device('cuda:0' if target.T.cuda.is_available() else 'cpu')
        self.to(self.device)
    

    def forward(self, state):
        state = target.T.tensor(state, dtype=target.T.float32).to(self.device)
...
class Fake_LearnFromOneSample(object):
    ...
    def __call__(self,model,target_model,episode):
        state = target.T.tensor([2], dtype=target.T.float32)
        pred = model(state)
        result_target = target.T.tensor([7], dtype=target.T.float32)
...
# begin to test if the back propogration is correctly calculated
@ddt
